...
def guardar_en_archivo():
    with open ("paciente.txt", "w", encoding="utf-8")as archivo:
        for paciente in pacientes_data:
            archivo.write(f"{paciente['nombre']}|{paciente['fecha de nacimienta']}| {paciente['edad']}|" 
...
def EliminarPaciente():
    seleccionado=TreeViw.selection()
    if seleccionado:
        ...
        messagebox.showwarning("eliminar Paciente", "no se a seleccionado ningunpaciente")
        return
